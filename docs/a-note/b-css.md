...
| 组合选择器 | 描述 |
| --- | --- |
| 相邻兄弟选择器 | A+B　B选择的元素在水平方向上紧随A选择的元素 |
| 普通兄弟选择器 | A`B　A选择的元素在B选择的元素之前，但不一定紧接 |
| 子选择器 | A>B　B是A的直接子元素 |
| 后代选择器 | A B　B是A的后代，不一定是直接子代 |
...
| background-attachment | scroll，fixed，local | 图像位置 |
| background-position -x/-y | top、right、bottom、left、% | 背景图像位置 |
| background-size | px、%、contain、cover | 背景图像大小 |
| background-origin | border-box、padding-box、content-box | 指定图像位置 |
| background-clip | border-box、padding-box、content-box、text | 绘制区域 |
| opacity | 0-1 | 不透明度，子元素将继承 |
| filter |  | 将滤镜添加给元素 |
| backdrop-filter | blur(px)、等滤镜 | 为元素后面的区域添加图形效果 |
|  | blur() 高斯模糊 |  |
...
| -webkit-line-clmp | number | 内容限定为指定行数 |
...
| border-collapse | collapse、separate | 决定表格边框的合与分 |
| border-spacing | px | 相邻单元格之间的距离 |
...
| 属性 | 值 | 描述 |
| --- | --- | --- |
| display |  | 设置元素的内部和外部显示类型，外部指块级或内联，内部指子元素 |
|  | none | 隐藏后元素不占位 |
| outside: | block | 块级元素 |
|  | inline | 行内元素盒子 |
|  | run-in | 取决于相邻元素的形态 |
...
|  | skew() | 倾斜 |
|  | matrix | 矩阵 |
| transform-orgin | center、top left、50px 50px | 更改变形原点 |
| transform-style | flat、preserve-3d | 元素位于平面或3d空间 |
| perspective | px | 指定观察者与z轴的距离，产生透视效果 |
| perspactive-origin | top、center、bottom、left、right、% | 指定观察者的位置 |
...
| 属性 | 值 | 描述 |
| --- | --- | --- |
| display | flex | 父元素设置容器 |
| flex-direction | row | 容器方向，从左到右 |
|  | row-reverse | 从右到左 |
|  | column | 从上到下 |
|  | column-reverse | 从下到上 |
| flex-wrap | nowrap | 不进行换行 |
|  | wrap | 必要时进行换行 |
|  | warp-reverse | 相反顺序换行 |
| flex-flow | row warp | 设置方向与换行 |
| justify-content | start | 在主轴线上的对齐方式，行首 |
|  | flex-start | 开头对齐 |
|  | flex-end | 末端对齐 |
|  | center | 居中 |
|  | space-around | 分散对齐 |
...
|  | space-evenly | 均匀分布 |
|  | stretch | 拉伸填充 |
| align-items | center | 交叉轴线上的对齐方式，十字轴上 |
|  | start | 起始边缘对齐 |
|  | end | 结束边缘对齐 |
|  | flex-start | 侧轴起点对齐 |
|  | flex-end | 侧轴终点对齐 |
|  | stretch | 拉伸填充 |
|  | baseline | 基线对齐 |
| align-content | start | 纵轴上内容项之间的对齐方式，起始边缘对齐 |
|  | end | 结束边缘对齐 |
|  | center | 居中对齐 |
|  | flex-start | 垂直轴起点对齐 |
|  | flex-end | 垂直轴末尾对齐 |
...
|  | sapce-evenly | 均匀分布 |
|  | stretch | 拉伸填充 |
| align-self |  | 单个盒子，在交叉轴上的对齐方式 |
| flex： |  |  |
| flex-grow | number | 相对于其他项目的增长倍数 |
| flex-shrink | number | 相对于其他项目的收缩倍数 |
...
| 属性 | 值 | 描述 |
| --- | --- | --- |
| display | grid | 父元素设置容器 |
| grid-template： |  |  |
| grid-template-rows | 200px 100px auto auto | 设置网格行数并初始化高度 |
| grid-template-columns | 100px 200px auto auto | 设置网格列数并初始化宽度 |
| grid-template-areas |  | 网格排列方式定义 |
| grid-auto-rows | px | 隐式网格行高 |
| grid-auto-columns | px | 隐式网格列宽 |
| grid-auto-flow | row、column、row dense | 自动布局排列方式 |
| grid-column-gap | px | 列间隙 |
| grid-row-gap | px | 行间隙 |
| justify-content | space-evenly | 容器中水平对齐方式 |
| justify-items | stretch、center、start、end | 内轴对齐方式 |
| align-content | 同弹性布局 | 容器中垂直对齐方式 |
| justify-self | stretch、center、start、end | 该元素到其格栅区域内联轴上 |
| align-self | 同弹性布局 | 所选项目的对齐方式，覆盖align-items |
| grid-area： |  |  |
...
| 属性 | 值 | 描述 |
| --- | --- | --- |
| scroll-color | track、thumb | 滚动条轨道和拇指颜色 |
