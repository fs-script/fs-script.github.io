...
| --- | --- |
| `nvm ls` | 列出所有可用版本 |
| `nvm ls available` | 列出已安装和可安装版本 |
| `nvm install 版本号` | 安装对应版本的 Node.js |
| `nvm use 版本号` | 切换对应版本的 Node.js |
...
| 常用命令 | 描述 |
| --- | --- |
| `npm init -y` | 初始化，在所处的目录快速创建 package.json 包管理的配置文件 |
| `npm install` | 安装项目中全部的依赖包，可简写为 `npm i` |
| `npm install 包名` | 安装指定的依赖包，可以指定版本 `包名@版本号` |
| `npm uninstall 包名` | 卸载指定的包 |
| `npm install -g 包名` | 将会在本地的电脑全局安装 |
| `npm install --save--dev 包名` | 仅在开发阶段需要使用，线上不再依赖的包使用，可简写为 `npm i -D 包名` |
| `npm config get registry` | 查看当前的下载源 |
| `npm config set registry=xxx` | 切换当前的下载源 |
| `npm login` | 登录服务器 |
...
| `fs.writeFile(file, data, [option], callback)` | 给指定的文件写入内容，该方法只能创建文件，不能创建路径 |
...
- 默认情况下会在命令执行的目录下拼接代码中的相对路径，可能会出现拼接错误，解决方法可以使用完整的绝对路径，但是不利于移植，不利于维护。在 Node.js 中 `__dirname` 表示当前文件所处的目录，可以解决路径拼接可能出现的错误，利于维护和移植。
...
| `app.use()` | 注册全局中间件 |
| `app.use(express.static('目录'))` | 创建一个静态资源服务器，存放静态资源的文件名不会出现在 URL 中 |

- 托管多个静态资源，即多次调用 `express.static()`，根据顺序查找需要的文件。
- 挂载路径前缀，访问时就需要使用挂载的前缀，`app.use('/前缀', express.static('/路径'))`
- nodemon 监听代码修改，自动重启项目，使用方式为： `nodemon app.js`
...
- 全局中间件：客户端发起的任何请求，到达服务器之后都会触发的中间件，使用 `app.use()` 注册全局中间件。
- 局部中间件：不使用 `app.use()` 定义的中间件。
...
- 使用中间件的注意事项：
  - 第一，一定要在路由之前注册中间件（除错误中间件外）；
  - 第二，客户端发送的请求可以调用多个中间件处理；
  - 第三，中间件的业务代码执行完毕之后要调用 `next()` 函数；
  - 第四， `next()` 函数之后不要再添加代码；
  - 第五， 连续调用多个中间件时，多个中间件共享 `res` 和 `req` 对象。
- 中间件的分类：
  - 第一类：应用级别的中间件，绑定到 app 实例上的中间件。
  - 第二类：路由级别的中间件，绑定到 router 实例上的中间件，`express.Router()`
  - 第三类：错误级别的中间件，用来捕获整个项目中发生的异常错误，防止项目异常崩溃的问题 `err, req, res, next`
  - 第四类：Express 内置的中间件，例如，`express.static` 快速托管静态资源，`express.json` 解析JSON格式的请求数据，`express.urlencoded` 解析 URL-encoded 格式的请求数据。
  - 第五类：第三方的中间件。
...
- CORS 跨域资源共享解决方法，CORS 是主流的跨域解决方案，JSONP 只支持 GET 请求。
...
- 简单请求：
  ...
  - HTTP 头部信息不超过以下几种字段：无自定义头部字段、`Accept`、`Accept-Language`、`Content-Language`、`DPR`、`Downlink`、`Save-Data`、`viewport-Width`、`Width`、`Content-Type`(只有三个值 `application/x-www-form-urlencode`、`multipart/form-data`、`text/plain`)
- 预检请求：
  - 上诉三者请求之外的请求方式；
  - 请求头中包含自定义头部字段；
  - 向服务器发送了 `application/json` 格式的数据。
- 正式通信之前会发送一个 OPTION 请求进行预检，获知服务器是否允许该实际请求，响应成功之后才会发送真正的请求，并携带真实数据。
- 两种请求的区别：简单请求只发送一次请求；预检请求会发生两次请求。

### 05 - REST API

- RESTful API 充满了具体的状态转移的 API
...
- `ctx` 是上下文对象，封装了 `req res`， `app.context` 是其创建原型，可以编辑为 `ctx` 添加其他属性。
