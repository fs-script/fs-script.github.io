...
-  路由分为三部分，客户端请求的类型、客户端请求的URL地址、服务器端的处理函数，`app.METHOD(PATH, HANDLER)` 
-  路由匹配按照定义的先后顺序进行匹配，请求类型和请求的URL同时匹配成功，才会调用对应的处理函数 
...
② 路由级别的中间件；绑定到router实例上的中间件，`express.Router()`
...
一、简单请求：① 请求的方式：GET POST HEAD 三者之一；② HTTP 头部信息不超过以下几种字段：无自定义头部字段、Accept、Accept-Language、Content-Language、DPR、Downlink、Save-Data、viewport-Width、Width、Content-Type(只有三个值 application/x-www-form-urlencode、multipart/form-data、text/plain)
二、预检请求：① 上诉三者请求之外的请求方式；② 请求头中包含自定义头部字段；③ 向服务器发送了 application/json 格式的数据
